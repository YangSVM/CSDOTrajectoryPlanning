...
class CL_CBS {
 public:
  ...
  bool search(const std::vector<State>& initialStates,
    ...
    start.constraints.resize(initialStates.size());     // each agent per constraints
    start.cost = 0;
    start.id = 0;
    ...
    for (size_t i = 0; i < initialStates.size(); ++i) {
      ...
      std::cout << "enter the low level search\n";  
    ...
    std::cout << "end of high level search: \n";
 ...
 private:
  ...
  struct LowLevelEnvironment {
    ...
    bool isSolution(
        const State& s, Cost g, State& path_end, 
        std::unordered_map<State, std::tuple<State, Action, Cost, Cost>,
                           std::hash<State>>& camefrom) {
      return m_env.isSolution(s, g, path_end, camefrom);
    }
