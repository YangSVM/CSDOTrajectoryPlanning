        ...
        num_of_agents(instance.getDefaultNumberOfAgents())
{
    ...
    {
        search_engines[i] = new HybriAStarInterface(instance, i);
    }
...
bool PBS::generateRoot()
{
    ...
    for (auto i = 0; i < num_of_agents; i++)
    {
        ...
        {            
            ...
            new_path = search_engines[i]->findOptimalPath(empty_higher_agents, paths, i);
            if ( new_path.empty() ){
...
bool PBS::getPaths(vector<Path>& solutions){
    if ( !solution_found){
        return false;
    }
    solutions.clear();
