        ...
        num_of_agents(instance.getDefaultNumberOfAgents()), T_plan(T_plan)
{
    ...
    {
        search_engines[i] = new WinHybriAStarInterface(instance, i, T_plan);
    }
    ...
    const vector<State>& starts)
{
    ...
    // ordered_agents: 从前往后是先Planning到后planning。所有的智能体的序号都在里面
    if (screen > 2)
    {
        cout << "Ordered agents: ";
    ...
    vector<int> topological_orders(num_of_agents); // map agent i to its position in ordered_agents。但是倒序。
    auto i = num_of_agents - 1;
    for (const auto & a : ordered_agents)
    {
        topological_orders[a] = i;      // 高优先级的数字大. 
        // 同时 n-topological_orders[i] 是 i在ordered_agents中的index。
        i--;
    }
    ...
    { // find conflicts where one agent is higher than high and the other agent is lower than low
        ...
        auto p = ordered_agents.rbegin(); // list(双向链表)的右起点
        // std::advance(iterator, n). 从迭代器开始叠加n。
        ...
        for (const auto & conflict : node->conflicts)
        {
            ...
            {
                // ensure a1 is lower order than a2.
                std::swap(a1, a2);
            }
    ...
    while(!to_replan.empty())
    {
        ...
        Path new_path;
        if(!findPathForSingleAgent(starts[a], *node, higher_agents, a, new_path))
        {
            delete node;
...
bool WinPBS::findPathForSingleAgent(const State& start, PBSNode& node, const set<int>& higher_agents, int a, Path& new_path)
{
    clock_t t = clock();
    new_path = search_engines[a]->findOptimalPath(start, higher_agents, paths, a);  //TODO: add runtime check to the low level
    ...
    // assert(!hasConflicts(a, higher_agents)); // Sampling errors can cause collisions
    return true;
}

// takes the paths_found_initially and UPDATE all (constrained) paths found for agents from curr to start
inline void WinPBS::update(PBSNode* node)
{
...
// bool ConfilctPairCheck(State& n1, State& n2, 
//     int id_start1, int node_check_step1, 
//     int id_start2, int node_check_step2
// ){
//     // n1 : target or vertex. n2: vertex.
//     // detect 
//     assert(node_check_step1 == 1 || 
//         (node_check_step1 == node_check_step2 && id_start1 == id_start2 ) );

//     vector<double> x1 = n1->GetXs();
//     vector<double> y1 = n1->GetYs();
//     vector<double> yaw1 = n1->GetYaws();

//     vector<double> x2 = n2->GetXs();
//     vector<double> y2 = n2->GetYs();
//     vector<double> yaw2 = n2->GetYaws();

//     // avoid calculate the disc more than one time if px's size = 1.    
//     assert(id_start1 < x1.size());
//     auto p_disc = vehicle_param_.GetDiscs(x1[id_start1], y1[id_start1], yaw1[id_start1]);

//     for ( int k = 0; k < node_check_step2 ; k++){
//         int id2 = id_start2+ k;
//         assert(id2 < y2.size());
//         auto traver_disc = vehicle_param_.GetDiscs(x2[id2], y2[id2], yaw2[id2]);
        
//         if ( node_check_step1>1 and k>0){    // avoid calculate the disc more than one time if px's size = 1.
//             assert(id_start1+k < x1.size());
//             p_disc = vehicle_param_.GetDiscs(x1[id_start1 + k], y1[id_start1 + k], yaw1[id_start1 + k]);
//         }
//         if ( p_disc.first.HasOverlap(traver_disc.first) || 
//                 p_disc.second.HasOverlap(traver_disc.first) || 
//                 p_disc.first.HasOverlap(traver_disc.second) || 
//                 p_disc.second.HasOverlap(traver_disc.second) 
//         ){
//             return true;  // collision and return not valid
//         }
//     }
...
bool WinPBS::SwapConflict(State& n1, State& n2) const {
...
bool WinPBS::TargetConflict(State& n1, State& n2) const {
    return n1.agentCollision(n2);
...
bool WinPBS::hasConflicts(int a1, const set<int>& agents) const
{
    for (auto a2 : agents)
    {
...
shared_ptr<Conflict> WinPBS::chooseConflict(const PBSNode &node) const
{
	if (screen == 3)
		printConflicts(node);
	if (node.conflicts.empty())
		return nullptr;
    return node.conflicts.back();  //为什么选择最后一个冲突
}
int WinPBS::getSumOfCosts() const
{
   int cost = 0;
...
inline void WinPBS::pushNode(PBSNode* node)
{
	// update handles
    open_list.push(node);
	allNodes_table.push_back(node);
}
void WinPBS::pushNodes(PBSNode* n1, PBSNode* n2)
{
...
void WinPBS::printPriorityGraph() const
{
    cout << "Priority graph:";
...
void WinPBS::printResults() const
{
...
void WinPBS::saveResults(const string &fileName, const string &instanceName) const
{
...
void WinPBS::saveCT(const string &fileName) const // write the CT to a file
{
...
void WinPBS::savePaths(const string &fileName) const
{
    if ( !solution_found ){
        cout<< "warning! solution not found";
        return;
    }

    vector<Path> plan_results;
...
void WinPBS::printConflicts(const PBSNode &curr)
{
...
string WinPBS::getSolverName() const
{
...
// change to a random restart root node to accelarate.
bool WinPBS::generateRoot( const vector<State>& starts)
{
...
/*inline void WinPBS::releaseOpenListNodes()
{
	while (!open_list.empty())
	{
		PBSNode* curr = open_list.top();
...
void WinPBS::clearSearchEngines()
{
...
bool WinPBS::validateSolution() const
{
...
void WinPBS::topologicalSort(list<int>& stack)
{
    stack.clear();
...
void WinPBS::topologicalSortUtil(int v, vector<bool> & visited, list<int> & stack)
{
...
void WinPBS::getHigherPriorityAgents(const list<int>::reverse_iterator & p1, set<int>& higher_agents)
{
...
void WinPBS::getLowerPriorityAgents(const list<int>::iterator & p1, set<int>& lower_subplans)
{
...
bool WinPBS::hasHigherPriority(int low, int high) const // return true if agent low is lower than agent high
{
    std::queue<int> Q;
    vector<bool> visited(num_of_agents, false);
    visited[low] = false;
...
bool WinPBS::getPaths(vector<Path>& solutions){
    if ( !solution_found){
        return false;
    }
    solutions.clear();
